...
export default function NationList() {
    ...
    // useEffect(()=> {fetch("http://localhost:3000/nations").then(response=> response.json()).then(json=> setNations(json))}, []);

    // useEffect(()=>{
    //     fetch("http://localhost:3000/nations").then(response =>{
    //         // http 상태 코드가 200번째가 아닌 경우
    //         if(!response.ok) {
