    ...
    return (
        <main>
            ...
            <form className="cont-input" onSubmit={handleSubmit}>
                <p className='txt-wannabe'>나는<input type="text" required value={expert} onChange={(event)=> setExpert(event.target.value)}/> 전문가가 될 것이다.</p>
                <p className='txt-time'>그래서 앞으로 매일 하루에 <input type="number" required value={traningTime} onChange={(event)=>settraningTime(event.target.value)}/>시간씩 훈련할 것이다.</p>
